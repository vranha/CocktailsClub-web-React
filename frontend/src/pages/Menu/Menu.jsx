...
export default function Menu({ expanded }) {
    ...
    const addProduct = (name, id, price) => {
        ...
        const quantity = document.querySelector(`.quantityInput-${id}`).value
            ? document.querySelector(`.quantityInput-${id}`).value
            : "1";

        if (productAdded) {
            setProductSelection([...productSelection].filter((product) => product.product !== name));
    ...
    const filterProduct = (e, filter) => {
        ...
        if (filter === "cocktail" || filter === "appetizer") {
            const filterProducts = products.filter((product) => product.type !== filter);
    ...
    useEffect(() => {
        navigator.geolocation.getCurrentPosition(function (position) {
            ...
            if (
                position.coords.latitude > 41.4309 &&
                position.coords.latitude < 41.4317 &&
                position.coords.longitude > 2.1562646 &&
                position.coords.longitude < 2.1568646
            ) {
